...
\ac{MFT} by \citep{higa2007} is another theory, which tries to address to the mismatch between the previously formed theories and the empirical evidence of media selection. \ac{MFT} is a combination of the factors from \ac{MRT} by \citet{daft1986} and \ac{SI} model by \citet{fulk1987}. In addition, \ac{MFT} adds factors from environment and resource limitations to the framework. By taking into account all the properties that affect on media selection, \ac{MFT} tries to provide a holistic view on media selection.

The main purpose of \ac{MFT} is to try to answer the simple question: why choose this medium but not that one \citep{higa2007}. The hypothesis of the theory is that the selection is done because other media are better \emph{fit} that another. Thus, the theory of media fitness defines media selection as follows: media selection is decided by the fitness of the media with the communication task needs, the communication user and user group, and the supporting environment in which the media being utilized \citep{higa2007}.

\ac{MFT} defines the fitness of the media by enumerating and grouping 14 properties related to fitness. 

Group I properties are requirements for the communication media. These properties are closely related to \ac{MRT} and \ac{MST}. The properties in this group are response time, security, sharing, retrieval, multiparty and expressive power as listed in table~\ref{table:mft_group1}.
...
Properties in group II related to communication participants, that is, the user group utilizing the particular communication media. The properties in this group are skill of using media, preference of media and group lifespan as listed in table~\ref{table:mft_group2}.
