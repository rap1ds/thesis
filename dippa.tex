...
Waterfall development relies heavily on contract negotiations. Contracts with fixed scope, time and budget do not spur feedback communication between customer and supplier. The reason for this is that there is no need for communication or feedback during the development, since all the details have been agreed and written down to the contract. It can even be argued, that the main purpose of the waterfall project, which relies on heavy contracts is not to build a great product, instead, to build a product that fulfills the contract.
