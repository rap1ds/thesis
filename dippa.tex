...
\hline
III-3 Bandwidth & How much bandwidth can be provided for communication media. \\
\hline
...
The main focus in this thesis are the properties of communication media itself, not for example the social aspect or environmental limitations affecting to the media selection. Thus, more weight is put on the group I properties. Since the social aspect is out of the scope of this thesis, the group II properties are almost ignored. Less weight is put on group III properties, however, they are still included with one exception. The property III-3 bandwidth is ignored, since it can be argued that in the organizations of today where the companies have very high speed internet connections, the bandwidth plays very little or no role in media selection.
...
\item Miten paljon arvostat palauttetyökalussa seuraavia ominaisuuksia:
  \begin{enumerate}
    \item Palautteen antamisen / saamisen jälkeen keskusteluyhteyden on jatkuttava samantien (esim. tarkentavat kysymykset)
    \item Välitön palaute / vastausaika \textit{(MRT, MST, MFT)}
    
    \item Persoonallisuus, eli palauttessa pitää näkyä ja tuntua oman kädenjälkesi
    \item Persoonallisuus \textit{(MRT)}
    
    \item Mahdollisuus ilmaista palaute mahdollisimman monilla tavoilla, esimerkiksi tekstillä, kuvilla, äänellä, videolla, eleille, äänenpainolla?
    \item Ilmaisun monimuotoisuus \textit{(MRT, MST, MFT)}
    
    \item Mahdollisuus ilmaista palaute ihmiselle luonnollisilla ilmaisutavoilla (esim kuva vs. teksti, video vs. ääni)
    \item Ilmaisun luonnollisuus \textit{(MRT, MST, MFT)}
    
    \item Mahdollisuus olla samanaikaisesti mukana useammassa kommunikaatiotilanteessa (esim face-to-face vs. IM)
    \item Samanaikaisuus \textit{(MST)}
    
    \item Mahdollisuus muokata, editoida ja viimeistellä palautetta ennen lähettämistä
    \item Mahdollisuus viimeistellä \textif{(MST)}
    
    \item Mahdollisuus palata annettuun palautteeseen ja tulkita se uudelleen
    \item Mahdollisuus uudelleenprosessointiin \textif{(MST)}
    
    \item Turvallisuus, tietojen pysyminen vain halutuilla kommunikaatiotahoilla
    \item Turvallisuus \textit{(MFT)}
    
    \item Helppo jaettavuus muille kuin kommunikaatiotapahtumasa mukana olleille
    \item Helppo jaettavuus \textit{(MFT)}
    
    \item Mahdollisuus indeksoida palaute myöhempää hakua varten
    \item Haku myöhempää käyttöä varten \textit{(MFT)}
    
    \item Mahdollisuus monen käyttäjän yhteiskäyttöön (esim. Google Docs)
    \item Monen käyttäjän yhtäaikainen käyttö \textit{(MFT)}
    
    \item Mahdollisuus antaa palautetta ajallisesti milloin vain (ei pelkästään työaikana)
    \item Saavutettavuus ajallisesti \textit{(MFT)}
    
    \item Mahdollisuus antaa palautetta sijainnillisesti mistä vain (ei pelkästään toimistosta)
    \item Saavutettavuus fyysisesti \textit{(MFT)}
    
    \item Kommunikaatiotyökalun käytön kustannukset
