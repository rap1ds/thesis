...
\DeclareAcronym{CAT}{
    short = CAT,
    long = compensatory adaptation theory,
    alt = Compensatory Adaptation Theory
}
\DeclareAcronym{CSS}{
    short = CSS,
...
The capturer captures the current DOM tree and associated \ac{CSS} style definitions. The captured DOM source and styles are sent to the Hannotaatio backend server and user is redirected to the editor site.

The editor is responsible of loading the saved DOM tree and C\ac{CSS}SS styles from backend server. The source is then rendered in a iframe container. On top of the iframe container, a drawing canvas is created. The canvas allows user to draw the feedback and save it for sharing.
